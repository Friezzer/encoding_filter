...
// Структура для хранения таблицы кодировки
struct EncodingMap {
    std::string name;
    uint32_t byte_to_unicode[256] = {0}; 
};

// Структура для хранения эталонных частот языка
struct LanguageFreq {
    std::string name;
...
struct LanguageMarkov {
    std::string name;
    size_t alphabet_size = 0; // Размер алфавита конкретного языка 
    
    std::vector<std::vector<double>> transition_matrix;
    
    // Карта перевода Юникода в локальный плоский индекс
...
public:
    ...
    // Главный метод: определяет кодировку
    // verbose=false для поблочного анализа
    std::string detect_encoding(const char* data, size_t size, bool verbose = true);

    void transcode_to_utf8_buf(const char* raw_data, size_t len, const std::string& encoding, std::vector<char>& out_buf) const;
...
private:
    ...
    double cosine_T_inf   = 0.83; // асимптота для длинных текстов 
    double cosine_T_floor = 0.60; // нижняя граница для коротких текстов
    double cosine_k       = 13.0; // скорость выхода на асимптоту
    ...
    static void unicode_to_utf8_buf(uint32_t codepoint, std::vector<char>& out_buf);
};